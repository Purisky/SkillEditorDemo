...
namespace Leopotam.EcsLite {
    ...
    public sealed class EcsPool<T> : IEcsPool where T : struct {
        ...
        #region static
        public static EcsPool<T> Pool
        {
            get {
                pool??= EcsWorld.Inst?.GetPool<T>();
                return pool;
            }
        }
        static EcsPool<T> pool;
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref T GetAdd(int entity) => ref Pool.Has(entity) ? ref Pool.Get(entity) : ref Pool.Add(entity);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref T SetAdd(int entity, T value)
        {
            ref var data = ref Pool.Has(entity) ? ref Pool.Get(entity) : ref Pool.Add(entity);
            data = value;
            return ref data;
        }
        #endregion
